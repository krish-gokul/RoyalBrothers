...
<div id="header">
	...
	<br>
	<form name="editform"  action="edit.php" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">

		<input class="custom-input" type="text" name="bike_model" id="bike_model" placeholder="Bike Model" required>
        ...
        <input class="custom-input" type="text" name="is_available" id="is_available" placeholder="Availability" required>
	...
	<button type="button " id="button" class="btn btn-primary" style="background-color:#f7cd46;color:black;border-color:#f7cd46;"> Edit</button>
	...
	<div id="edit">
		...
		<caption><h3>Edit Entries</h3></caption>
  		<tr>
    		<th>Bike Model</th>
    		<th>Bike Image</th>
    		<th>Availability </th>
  		...
  		<tr>
    		...
    		<td><?php echo $row["Bike_Image"]; ?></td>
    		<td><?php echo $row["Is_Available"]; ?></td>
    		<?php echo '<td><a href="update.php?id='.$row["Bike_Id"].'"><button class="btn btn-danger"> Edit </button></a> </td>';?>
	...
	{
    	var bike_model = document.forms["editform"]["bike_model"].value;
    	var fileToUpload = document.forms["editform"]["fileToUpload"].value;
    	var is_available = document.forms["editform"]["is_available"].value;
    	if (bike_model == "") {
        	alert("Bike Model must be filled out");
        	return false;
    	}
    	if(fileToUpload == "")
    	{
    		alert("Bike image should be uploaded");
    		return false;
    	}
    	if(is_available == "")
    	{
...
// if everything is ok, try to upload file
    ...
    if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
       // echo "The file ". basename( $_FILES["fileToUpload"]["name"]). " has been uploaded.";
        ...
        $query = "INSERT into bike (Bike_Model, Bike_Image, Is_Available) VALUES ('$bike_model','$fileName', '$is_available')";
        $result = mysqli_query($con,$query) or die(mysqli_error($con));
    } else {
